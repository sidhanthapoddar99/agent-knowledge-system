...
The underlying data already exists — every loaded page carries `fileType` (`'md' | 'diagram' | 'artifact' | …`, `agent-ks-engine/src/parsers/types.ts`), set by `src/loaders/diagram-pages.ts` and `src/loaders/artifact-pages.ts` — but the sidebar tree model drops it before render.
