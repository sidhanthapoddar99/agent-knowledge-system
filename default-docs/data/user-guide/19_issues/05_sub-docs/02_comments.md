...
Comments are **append-only in practice** — they record history, so they don't get renumbered or rewritten. Keep them a **lean evolution log** (what changed, status shifts, hand-offs), not a forum: the debate that produced a decision lives in [Brainstorm](./06_brainstorm.md). The tripwire: a comment is **a couple of lines plus a pointer** — the moment you're writing a second paragraph you're in the wrong section (debating → Brainstorm, specifying → [Notes](./04_notes.md)); link instead of inlining.
...
Pure markdown — no length cap, though most comments are a line or two. For design deliberation, use **[Brainstorm](./06_brainstorm.md)** — comments record *that* something happened (a decision, a status shift, a hand-off), never the debate that produced it.
...
The live editor + planned `/issues` skill automate all of this — see [Using with AI](../09_using-with-ai.md).
...
- [issue.md](./01_issue-md.md) — the initial pitch
- [Subtasks](./03_subtasks.md) — atomic work units with their own state
- [Agent Log](./05_agent-log.md) — audit trail for AI iterations
