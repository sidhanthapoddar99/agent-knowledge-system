...
- **Promotion is the lifecycle.** When work on a step begins, it **graduates to its own issue** (`docs-guide move` any relevant notes across; see [43_moving-restructuring.md](../40_operations/43_moving-restructuring.md)). The original subtask stays in the index as the pointer ("→ promoted to `<new-issue-id>`") and flips to `review`/`done`; the new issue carries the actual planning + execution.
