...
> shipped in [the section loop](../010_lp_implement-sections/01_summary.md).
>
...
[the loader survey](../010_lp_implement-sections/02_working/011_research-loader-shapes.md)
existed. Opened as an experiment (`ex`) rather than a refactor because the
intended outcome was a decision, not a change.

# Todo

- [x] [Scope the surfaces the one-pass reader has to cover](./02_working/010_scope-the-surfaces.md)
      — eight section shapes, and the two that make a single pass awkward
- [~] [Prototype and probe](./02_working/020_one-pass-probe.md) — the prototype ran
...
The reasoning is in [the probe](./02_working/020_one-pass-probe.md); the shape that
shipped instead is
[the section loop](../010_lp_implement-sections/01_summary.md).
...
[the probe](./02_working/020_one-pass-probe.md) tints its prefix number — that is the
...
What not to retry is in [the handover](./03_debrief/01_handover.md).
