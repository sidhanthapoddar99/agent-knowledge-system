...
| Flag | Effect |
|---|---|
| `--dpr 2` | The largest saving. A retina capture is at 2× scale. Halving it cuts the pixel count to a quarter, with no loss on a 1× display |
...
`--target-size` steps quality down from 80 to 30 until each file fits. If quality 30 is still over budget, the tool warns. Then use `--trim` to crop an empty background, or `--max-dim` or `--scale` to cut pixels. Cutting pixels saves more than lowering quality.
...
| jpg | photos for legacy targets | never for text, because it leaves ringing marks on letter edges |

## Pitfalls

| Pitfall | Rule |
|---|---|
| `--colors N` helps PNG and hurts lossy webp and avif | Reducing colours makes hard edges. The codec must encode those edges, so the file grows. The tool warns. Use `--colors` only with `--format png` |
| Resolution saves the most | `--dpr`, `--scale` and `--trim` shrink more than a quality change. Cut pixels first |
...
Do not trace a raster image to SVG. Tracing turns letters into fuzzy outlines, and the file is usually larger than a colour-reduced webp. A vector figure with real text comes only from the capture step: `page.pdf()`, or a DOM-to-SVG capture. That is not the job of `agent-ks img`.
