...
- **[the Astro 7 comparison](./notes/astro-7-vs-go/01_comparison.md)** — the two live options set side by side across the eleven items that actually matter here: RAM, live reload and partial builds, size, page load, start time, multi-user editing, control, external layouts, rendering quality, production-like feel, caching. Go wins six, Astro 7 wins three, two are a tie. **Read this before the architecture notes below** — the audit it draws on refuted much of their performance case, and one measurement (Astro in production server mode) is still missing and decides three rows.
