...
- [ ] `src/loaders/engine-version.ts` — `ENGINE_VERSION = "0.7.0"`,
      `MIN_CONTENT_VERSION = "0.7.0"`, and a small `compareVersions()` (numeric
      per segment; gate compares major.minor only).
- [ ] Gate in `loadSiteConfig()` (`src/loaders/config.ts`): read top-level
...
- [ ] Declare `engine_version: "0.7.0"` in `default-docs/config/site.yaml` (with
      an explanatory comment) so the dogfood project passes its own gate.
- [ ] `/docs-init` scaffold writes `engine_version` into new projects.
- [ ] Verify: build green with the declaration; temporarily drop/raise it and
