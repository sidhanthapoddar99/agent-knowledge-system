...
- [ ] **`tooltip.ts` script page.** `15_scripts/` documents each browser script but `tooltip.ts` is missing entirely — add `NN_tooltip.md` (mechanism: singleton `.ui-tooltip`, overflow detection, `data-tip-always`, cursor anchoring) and a row in `15_scripts/05_overview.md`'s script table.
- [ ] **CLAUDE.md pointer.** One short paragraph/line in `CLAUDE.md` referencing the UX-standards page — pointer only, no inlined content.
- [ ] **No legend duplication.** The status/type symbol *legend* lives in the Guide panel + user-guide (subtask `40_guide-legend.md`); this page explains *why*, not *what each icon is*.
