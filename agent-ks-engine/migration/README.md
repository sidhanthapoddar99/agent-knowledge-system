# agent-ks-engine/migration/ — content-format migrations
...
2. Run **each** `agent-ks-engine/migration/` script in `(X, Y]` in version order: detect →
...
`agent-ks-engine/migration/<new-version>_*.py`. The floor (`MIN_CONTENT_VERSION` in
`agent-ks-engine/src/loaders/engine-version.ts`) moves **only for breaking
...
`MIN_CONTENT_VERSION` in `agent-ks-engine/src/loaders/engine-version.ts`) is
