...
|---|---|---|
| Rank magnitudes, small to large | bar / column (grids take a **heatmap**) | one-hue sequential |
...
| Read an ordered-scale split (Likert, agree↔disagree) | **diverging stacked bar** pinned to its neutral center | diverging |
...
- **Categorical earns its keep only when the series themselves are the story** — and
  ...
  one.
- **Emphasis is the form people skip and shouldn't.** The series that matters takes
  the accent while the rest sink into de-emphasis gray — usually the honest response
  to "this chart is too busy."
...
| 7–8 | the hard token ceiling; beyond it, sweep the tail into "Other", break out small multiples, or double-encode (hue × shape) |
