...
A plan stores no status of the work, so it cannot drift from the work. No other file states order. A log that lists its rounds as a schedule is a copy of the plan, and a copy drifts.
...
`plans/` holds plan folders and nothing else. `overview.md` holds all five sections of the [template](03_writing.md): the goal, what goes where, stage order, plan-level decisions, and the result. The skeleton is [plan-overview.md](../../agent-ks-cli/templates/plan-overview.md). Say "stage", not "section". A section is a top-level issue folder.
...
The frontmatter holds `title`, `status`, `outcome`, `notes`, `who` and `subtasks:`. The body holds all five template sections. The skeleton is [plan-stage.md](../../agent-ks-cli/templates/plan-stage.md).
...
`outcome` says what "done" means here. `notes` says why it sits here, what it waits on, what would surprise a reader. Both are one line. Both render as inline markdown: a link, `code`, emphasis. Point with a link, never with a number. "Blocked on 14" means nothing once stage 14 is renumbered.

Keep a stage short, but not thin. A stage with three unexplained checkboxes leaves its reasoning unrecorded. Use [the qna skill](../../agent-ks-qna/SKILL.md) to get the why, the outcome and the open questions out of the user before the stage runs.

## The `subtasks:` list

A `subtasks:` entry is one plain markdown link. Nothing before it. Nothing after it. The validator, `agent-ks check issues`, errors on anything else. The path is the truth. The link text is a reading aid. The renderer resolves the path and shows the subtask's live title and status. A path that resolves to nothing is a validator error, and the plan page lists it in red.

`subtasks:` is the only structured list a stage carries. An `agent-logs:` key is a validator error. Link a run from `03 References` as an ordinary link with an ordering label. `agent-ks move` rewrites such a link, and the link can sit in a sentence that says why the run matters.

## Stage status

A stage uses the same eight values as everything else. A stage's status describes the schedule, never the work. The subtasks it references render their own status under it.
...
| `in-progress` | the stage that is being worked on. One at a time. A second one usually means you need two plans |
...
A stage does not wait for its subtasks to reach `done`. Set the stage from the schedule's point of view. Who may set `done` or `dropped` is in [closing authority](02_lifecycle.md).
...
The active plan is the highest-numbered plan whose status is not Closed. The engine derives it. It is never stored. One active plan at a time is the convention. The sidebar lists plans in prefix order, lowest first, and marks the active one in bold. `agent-ks check issues` warns when two plans are open.
...
A plan that cannot take in a discovery gets abandoned mid-run. That is why you may add a stage. Questions about one stage go in that stage's `05`. Questions that outlive every plan go in `notes/`.
...
Run `/agent-ks-index-check <path>` to check an index against its folder. Run it on a plan when you doubt that its stages still match the subtasks and logs they name.
