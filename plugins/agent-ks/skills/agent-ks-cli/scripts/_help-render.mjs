...
/** Every manifest entry whose verb is `token` (for an "ambiguous verb" message). */
export function entriesForVerb(token) {
  return MANIFEST.filter((c) => c.verb === token);
}

/**
 * Resolve a command token to a manifest entry: "docs-list", "issue list", a
 * top-level verb, or a verb that exists in exactly one group ("new-round").
 * A verb that exists in several groups ("list") resolves to nothing.
 */
export function resolveEntry(tokenA, tokenB) {
  const two = tokenB ? `${tokenA} ${tokenB}` : null;
  const direct = (two && bySubcommand[two]) || byBin[tokenA] || bySubcommand[tokenA] || null;
  if (direct) return direct;
  const sameVerb = entriesForVerb(tokenA);
  return sameVerb.length === 1 ? sameVerb[0] : null;
}
