...
Transitions are not enforced. Any jump is legal. The UI filters by category and shows the status as the badge.
...
|---|---|---|
| an issue or a subtask | the user only. Your ceiling is `review`, `input-needed` with the question inline, or `superseded` with its `→` line | closing signs off the work. The user inspects the artefact and flips it |
...
| 2. Stuck → `input-needed`, not `blocked` | Write the question inline in the item body. Reserve `blocked` for a dependency on another issue or subtask, named in prose |
