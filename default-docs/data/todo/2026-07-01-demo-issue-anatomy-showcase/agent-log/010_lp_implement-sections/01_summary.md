...
> all ship. [The sizing round](./02_working/050_sizing-tweaks.md) sits at
> `input-needed` — it needs an eye on a real screen, which no headless check can
> give. [The codec migration](./100_wf_codec-migration/01_summary.md) is a child log
...
- [x] [Read the loader and land the reader](./02_working/010_loader-reader.md) —
      named every hard-coded section string, then replaced them with one registry
      entry per section
- [x] [Survey how comparable loaders are shaped](./02_working/011_research-loader-shapes.md)
      — a producer file: four loaders compared, and why none of them round-trips
      a prefix
- [x] [The tradeoff write-up](./02_working/012_discuss-tradeoffs.md) — a producer
      file: why the registry won over a per-section switch
- [x] [Sidebar and tree](./02_working/020_sidebar-and-tree.md) — nesting, counts,
      and the depth cap made visible instead of silent
- [x] [Measure the build, before and after](./02_working/030_bench-build-time.md) —
      the numbers, with units
- [x] [Codec research](./02_working/040_research-codecs/01_findings.md) — one
      producer, two artifacts, which is why it earns a folder rather than two
      loose files
- [~] [Sizing tweaks](./02_working/050_sizing-tweaks.md) — landed, then stopped on a
      question only a person can answer. Not blocked: waiting on a look
- [ ] [Codec migration](./100_wf_codec-migration/01_summary.md) — a **child agent
...
[the round that landed it](./02_working/010_loader-reader.md) was actually for.
...
[the benchmark round](./02_working/030_bench-build-time.md). A run claiming an
improvement without numbers has not finished, and this one has them.

**What leaves the run.** Two things went to
[the handover](./03_debrief/01_handover.md) rather than blocking it: the
shared-fixture problem the next sweep inherits, and
[two questions for review](./03_debrief/02_questions-for-review.md) the loop could
