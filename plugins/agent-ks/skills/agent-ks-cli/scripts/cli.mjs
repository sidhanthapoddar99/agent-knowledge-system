...
// the grouped listing — `agent-ks`, `agent-ks --help`, and `agent-ks -h`
// all behave like `agent-ks help`. (A real command always supplies a group or
...
if (!match) {
  ...
  for (const c of MANIFEST) {
    ...
    console.error(`  ${c.bin}  (agent-ks ${sub})`);
  }
  process.exit(2);
}
...
// grammar, --json, exit codes) lives in the manifest + references/contract.md, so a Python
