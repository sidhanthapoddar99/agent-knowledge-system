...
└── subtasks/                      ORDERED pointers — one per step of the phase
    ├── 00_ideation.md             closed (done early)
...
- **Promotion is the lifecycle.** When work on a step begins, it **graduates to its own issue** (`docs-move` any relevant notes across; see [43_moving-restructuring.md](43_moving-restructuring.md)). The original subtask stays in the index as the pointer ("→ promoted to `<new-issue-id>`") and flips to `review`/`closed`; the new issue carries the actual planning + execution.
