...
| What | Where |
|---|---|
| The built-in default theme | `astro-doc-code/src/styles/`; read-only. Referenced as `@theme/default` |
...
Then `site.yaml`: `theme: "purple"` with `theme_paths: ["@themes"]`. Restart `./start` once for a new folder; later CSS edits hot-reload.
...
The default theme's files, by role: `color.css` (colours, both modes), `font.css` (families, scale, semantic tokens), `element.css` (spacing, radii, shadows, transitions, dimensions), `reset.css`, `markdown.css`, `navbar.css`, `footer.css`, `docs.css`, `blogs.css`. A child overrides a subset by shipping a file of the same name.
...
Every theme must define, or inherit, every variable in `astro-doc-code/src/styles/theme.yaml → required_variables`. A variable is on the list if and only if a shipped layout reads it. The list does not complete scales for tidiness: `--font-weight-normal` is required and `--font-weight-bold` is not. `agent-ks theme tokens --json` prints the live values for light and dark. The groups:
...
A theme may declare extra variables for its own files. Nothing else reads them. To make a new variable public, add it to `required_variables`; the validator then enforces it on every theme.
...
The loader checks at load time. Errors block the theme; warnings show in the dev toolbar's error logger and let it load.

| Message | Cause | Fix |
|---|---|---|
| Theme "x" not found | `theme:` names a folder outside `theme_paths` | Fix the name, or add the folder to `theme_paths` |
| lists file "y" but file does not exist | `files:` names a missing file | Create it or drop the entry |
| missing required variable | Standalone, `override` or `replace` theme without it | Declare it. With `merge` a gap inherits silently |
| Circular theme inheritance | Two themes extend each other | Point one at `@theme/default` or `null` |
| override_mode "z" is not valid | Typo | `merge`, `override` or `replace` |
