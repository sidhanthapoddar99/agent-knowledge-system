...
**Skills (10).** `agent-ks-config` owns setup and configuration — a new project from the starter template, a new top-level section, `config/*.yaml`, `.env`, path aliases, themes, layout styles and custom layouts, custom pages, and format migrations; it is rare work, so it is a set of complete references (`01_new-project` … `08_migrations`). `agent-ks-docs` owns pages *inside* a docs section — `NN_` prefixes, `settings.json`, frontmatter, links, diagram and artifact pages, images (`writing.md`, `docs-layout.md`, `images.md`); trigger it for anything under a docs section of `default-docs/data/`. `agent-ks-blog` owns posts under `data/blog/`, one file. `agent-ks-issue-logs` owns agent logs: when a run earns one, the six kinds, the file set of each. `agent-ks-qna` scopes a subtask or a stage by question and answer before a long run — the why, done-when, guardrails, decisions with reasons, freedom level — in story mode (a dictated brief) or interview rounds, and writes the answers into the subtask's own sections. `agent-ks-cli` is the CLI contract and the file templates. `agent-ks-quick-idea-note` and `agent-ks-index-check` are commands. `agent-ks-issues` is the self-contained issue-tracker skill — the whole anatomy (`brainstorm/` · `notes/` · `subtasks/` · `agent-log/` activity folders · `agent-memory/` · flat `comments/` · `glossary.md`), the creation threshold rules + issue dump, the 8-status / 4-category lifecycle (statuses fixed in framework code, shared by issues + subtasks under one `status` field) + AI rules, and its own tracker-flavoured writing reference; it also fires on a discussion against a tracked issue (an audit, refactor or loop run routes to `agent-ks-issue-logs`), keeps always-on agent-memory, and treats discussion as explicit-save-only (offer when dense, never auto-save). For anything under `default-docs/data/todo/`, load `agent-ks-issues`. `agent-ks-artifacts` is the artifact skill — building self-contained HTML artifacts (reports, dashboards, data viz, design systems, variation sets) as `NN_`-prefixed `.html` pages served at `/artifacts`, with the `.meta.json` sidecar, theme modes, and the verify gate.
...
**Slash commands** (every one is a skill folder, so Codex reads them too):

| Command | Use |
|---|---|
| `/agent-ks-config` | Bootstrap a new agent-knowledge-system project from zero — interactive: scope (whole repo vs subfolder) → site identity → confirm → copies the template, patches `CLAUDE.md`. Prints the framework-clone command at the end. |
| `/agent-ks-config section <name>` | Scaffold a new top-level section under `data/` — creates `settings.json` + starter page, optionally registers it in `site.yaml`. |
...
| `/agent-ks-index-check [path]` | Check an index (a plan, a log's `00_index.md`, a subtask group leaf) against the files it names. Reports only. |
