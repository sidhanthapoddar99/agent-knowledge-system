...
| `agent-ks issue new-agent-log <id>` | Scaffold an agent log — `settings.json` + `01_summary.md`. `02_working/` and `03_debrief/` appear when there is something to put in them; `--parent` nests a child log, whose prefix is `≥ 100` | `--kind <code>` · `--name <slug>` · `--parent <path>` (child log) · `--group <a[/b]>` · `--prefix <NNN>` · `--goal <text>` · `--json` · `--tracker` |
