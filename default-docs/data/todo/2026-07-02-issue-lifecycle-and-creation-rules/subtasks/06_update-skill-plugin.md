...
- [x] `skills/doc-issues/SKILL.md` — the router's "Lifecycle — states & AI rules"
      ...
      mentions.
- [x] `skills/doc-issues/references/02_settings.md` — **delete the "Assignees double
...
- [x] `skills/doc-issues/references/03_vocabulary.md` — the fixed enum + category
      grouping; root settings.jsonc's shrunken role (colors only); `wip` label
      deprecation note.
- [x] `skills/doc-issues/references/23_subtasks.md` — "state follows the same
...
- [x] `skills/doc-issues/references/42_updating.md` — creation flow's state
      guidance; `docs-guide issue set-state` examples under the new vocabulary.
- [x] `skills/doc-issues/references/41_searching.md` + `24_agent-logs.md` +
      worked examples (`61`–`64`) — sweep for `open + review` default-scope and
      old-state mentions; update to category language.
- [x] `skills/documentation-guide/` — sweep its references for lifecycle mentions
      (the sibling pointer text, `cli-toolkit.md` set-state examples).
- [x] Repo `CLAUDE.md` — tracker-mental-model paragraph + doc-issues skill
      description: new states/categories, drop any assignee-derivation mention.
- [x] Bump plugin version once; rsync to installed cache; `diff -r` verified
      identical.
