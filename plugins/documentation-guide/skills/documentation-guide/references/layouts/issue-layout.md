...
- **Subtask numbering — `NN_` or `NNN_`, gap-spaced** (applies to leaf files **and** grouping folders). Subtasks use the **same shared ordering-prefix grammar as docs** (2–5 digits, ordered by *numeric value*, so widths coexist — `01_` and `010_` sort as 1 and 10; see `references/layouts/docs-layout.md`). Unlike docs, where 2-digit is the near-universal default, **the issue tracker treats both 2- and 3-digit as conventional** and reaches for 3-digit far more freely:
  ...
  **Gap-number** either width (step 10, or 5 for denser sets) so a new subtask slots *between* two existing ones without renumbering: drop `015_` between `010_` and `020_`. **Separator:** `_` is canonical; the loader also tolerates a legacy `-` (`00-foo`) in existing folders — prefer `_` for anything new. (Comments and agent-logs are auto-numbered sequentially by the CLI — 3-digit — so you never hand-number them and gaps don't apply there.)
- **`notes/` and `agent-log/`:** a numeric prefix is **optional** here (folders and files) — but when you do number them, `NN_` and `NNN_` are both good conventions, using the *same* shared grammar (2–5 digits, ordered by value, `_` canonical / legacy `-` tolerated). Gap-number `notes/` when a fixed reading order matters (e.g. `010_context.md`, `020_design.md`); leave them unprefixed when order doesn't matter. For `agent-log/`, `NNN_` is the good (and CLI-written) convention: files that start with a number have it parsed as the iteration sequence; otherwise the loader assigns one by sort order within the leaf folder.
