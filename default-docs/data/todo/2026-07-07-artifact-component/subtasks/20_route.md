...
- [ ] **Create `src/pages/artifacts/[...path].ts`** by cloning
...
- [ ] **Confirm route priority over the catch-all.** The static `artifacts/`
...
- [ ] **Reserve the slug in server-mode routing.** Add `'artifacts'` to
...
- [ ] **No alias change needed.** A serving route is not a layout or `paths:`
...
- [ ] **Close the cache-busting handshake with the embed.** The iframe `src`
...
- [ ] **Verification.** With an artifact present (`NN_demo.html` from `10`), run
