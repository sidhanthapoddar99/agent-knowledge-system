...
/**
 * Resolve one subtask selector to the subtask records it names.
 *
 * A selector is a number (`10`), a slug (`010_top-layer-tests`), a slug prefix
 * (`010`), a clean name (`top-layer-tests`), a file name, or a path. A path is
 * resolved against the issue folder, each of `fromDirs`, the tracker root and
 * the working directory. Returns every match; the caller decides what zero or
 * more than one means. Nothing here guesses.
 */
export function resolveSubtaskSelector(trackerPath, issueId, selector, fromDirs = []) {
  const sel = String(selector ?? '').trim();
  if (!sel) return [];
  const subs = readIssueSubtasks(trackerPath, issueId);
  if (sel.includes('/') || sel.includes('\\') || sel.endsWith('.md')) {
    const bases = [path.join(trackerPath, issueId), ...fromDirs, trackerPath, process.cwd()];
    const wanted = new Set(bases.map((b) => path.resolve(b, sel)));
    if (path.isAbsolute(sel)) wanted.add(path.resolve(sel));
    return subs.filter((s) => wanted.has(s.filePath) || (!sel.includes('/') && s.fileName === sel));
  }
  if (/^\d+$/.test(sel)) {
    const n = parseInt(sel, 10);
    return subs.filter((s) => s.sequence === n);
  }
  return subs.filter((s) =>
    s.slug === sel || s.slug.startsWith(`${sel}_`) || parseOrderPrefixLoose(s.slug).cleanName === sel);
}
...
export function readAgentLogRounds(runDir) {
  ...
  for (const e of fs.readdirSync(runDir, { withFileTypes: true })) {
    ...
    const valid = prefix !== null && prefix >= 10;
    let fm = {};
    try { fm = frontmatterData(fs.readFileSync(abs, 'utf-8')); } catch { /* malformed — reported by check */ }
    out.push({
      name: base,
      fileName: e.name,
      filePath: abs,
      prefix,
      round: valid ? Math.floor(prefix / 10) : null,
      report: valid ? prefix % 10 : null,
      title: fm.title ? String(fm.title) : base,
      status: normalizeStatus(fm.status) ?? (fm.status ? String(fm.status) : null),
      agent: fm.agent ? String(fm.agent) : null,
    });
  }
  return out.sort((a, b) => (a.prefix ?? Infinity) - (b.prefix ?? Infinity) || a.name.localeCompare(b.name));
}

function makeAgentLogFolder(abs, name, groupPath, match) {
  let entries = [];
...
export function readIssueAgentLogFolders(trackerPath, issueId) {
  ...
  const out = [];
  const walk = (absDir, groupPath) => {
    let entries;
    try { entries = fs.readdirSync(absDir, { withFileTypes: true }); }
    catch { return; }
    const dirs = entries.filter((e) => e.isDirectory())
      .map((e) => ({ name: e.name, position: parseOrderPrefixLoose(e.name).position }))
      .sort((a, b) => (a.position ?? Infinity) - (b.position ?? Infinity) || a.name.localeCompare(b.name));
    for (const d of dirs) {
      const abs = path.join(absDir, d.name);
      const m = d.name.match(AGENT_LOG_RUN_RE);
      if (m) out.push(makeAgentLogFolder(abs, d.name, groupPath, m));
      else if (groupPath.length < MAX_SUBFOLDER_DEPTH) walk(abs, [...groupPath, d.name]);
    }
  };
  walk(root, []);
  return out;
