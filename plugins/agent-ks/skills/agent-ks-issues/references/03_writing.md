...
| agent log files | guided per kind: [agent-ks-issue-logs](../../agent-ks-issue-logs/SKILL.md) | `title`. Status lives in the log's `settings.json` | one `log-index-<kind>.md` per kind in the cli skill's `templates/` |
