...
- [ ] **Parse `color` frontmatter.** Extend the notes and agent-log frontmatter schema with an optional `color: <css-color>` field. Accept any string the browser accepts as a CSS color — named colors (`red`, `salmon`), hex (`#e06c75`, `#f00`), or longer forms. No vocabulary token system, no theme-alias mapping. The user picks; the framework doesn't impose semantics.
- [ ] **Apply tint to the entry's icon in `SubdocTree.astro`.** When `color` is present, render the entry's icon tinted to that value. **Only the icon** is coloured — the label text, row background, and borders all stay default. Implement via inline `style="color: <value>"` on the icon SVG / element (or a CSS custom property on the row that the icon's `fill` / `color` consumes).
- [ ] **Default behaviour (no `color` set).** Icon uses the default theme colour (`--color-text-secondary` or whichever the tree currently uses). No regression for existing entries.
- [ ] **Test fixtures.** Add at least one note and one agent-log entry with a named color (e.g. `red`), one with a hex (`#7aa2f7`), and one with `color` omitted — to exercise the present-named, present-hex, and absent paths.
- [ ] **Update docs.** Document the `color` field in the user-guide pages for notes and agent-log (under `default-docs/data/user-guide/19_issues/05_sub-docs/`). Make clear: it's optional, the user assigns its meaning (status / category / urgency / personal taste — the framework doesn't care), and only the icon is tinted.
- [ ] **Update plugin skill.** Add a one-liner to `plugins/documentation-guide/skills/documentation-guide/references/issue-layout.md` so agents know about the field and don't strip / overwrite it when editing frontmatter.
