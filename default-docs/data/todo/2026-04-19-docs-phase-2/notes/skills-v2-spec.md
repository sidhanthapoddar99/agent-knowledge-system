...
- **Subtask owns the job.** What to do, done when, what came out, what went wrong, what to watch for, decisions, Q&A. Five `#` sections stay. Standard `##` sub-heads: `Questions` and `Done when` under 01; `Result` and `Agent log` under 02; `NN <decision>` under 04; `Issues hit`, `Watch out`, `NN <point>` under 05. To-dos nest as deep as needed. `Done when` is a plain list. `## Agent log` is `none` or exactly one link; the validator warns otherwise.
