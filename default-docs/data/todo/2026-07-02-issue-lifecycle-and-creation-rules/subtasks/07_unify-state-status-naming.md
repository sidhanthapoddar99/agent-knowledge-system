...
- [x] **Value migration in the SAME script** (decided) — `closed` → `done` and
...
- [x] **Update the code** — issues loader (`src/loaders/issues.ts`) and the
      `docs-guide` CLI (`issue subtasks`, `issue set-state`, `check issues`) read the
      canonical field on both issues and subtasks.
- [x] **Error/warning on the legacy name** — when a file still uses the old field,
...
- [x] **Migration script** — converts all existing subtask frontmatter to the
...
- [x] **Migration check** — after the script runs, a validation pass
...
- [x] **Propagate the naming** — docs (`19_issues`), `guide.ts`, and the `doc-issues`
