...
import { manifestJson, renderCommandDetail, renderList, resolveEntry, entriesForVerb, usageLine } from './_help-render.mjs';
...
if (positionals[0]) {
  ...
  if (!entry) {
    ...
    const same = entriesForVerb(positionals[0]);
    if (same.length > 1) console.error(`  the verb exists in several groups: ${same.map(usageLine).join(' · ')}`);
    process.exit(2);
  }
