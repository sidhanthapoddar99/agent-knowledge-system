...
The plugin ships **`agent-ks check blog`** (on your `PATH` after install) — runs structural checks against the blog folder so you don't have to eyeball it.
