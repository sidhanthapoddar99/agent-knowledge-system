...
- [x] **Remove from the schema.** Wherever the issue settings shape is typed or documented (look under `agent-ks-engine/src/loaders/issues.ts`, any `IssueSettings` interface, JSON schema if one exists). Field disappears from the type system.
...
- `agent-ks-engine/src/loaders/issues.ts` and any `IssueSettings` type definition
- `agent-ks-engine/src/layouts/issues/default/parts/index/IssuesTable.astro`, `IssuesCards.astro`, `FilterBar.astro`, `PresetStrip.astro`, `scripts/index/types.ts`, `filters.ts`, `groups.ts`, `client.ts`
