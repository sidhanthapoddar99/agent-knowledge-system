...
Nobody hand-picks chart color. Each color in a chart holds exactly one job. No palette is legal until it passes the checks in this file. The checks let you swap a palette safely and let one method drive any design system's ramps. The procedure that calls this file is [dataviz.md](dataviz.md). The values for this framework are in [palette.md](palette.md).
...
|---|---|---|
| Categorical | Which series: identity | Eight hues in a frozen order, dealt one by one, never recycled |
| Ordinal | A rung in an ordered run: funnel step, tier, bucket | One hue stepped through lightness; the lightest rung keeps ≥ 2:1 on the surface |
...
Categorical or ordinal: test whether a reorder rewrites the meaning. When it does (funnel stages, S/M/L tiers, age brackets), the data is ordinal; give it a one-hue ramp. When it does not (products, teams, regions), the data is nominal categorical: one series wears slot 1's hue with no legend box, and N series take slots 1..N. Never shade nominal bars by their value; that spends the identity channel to restate bar length.

## Pick the job

- **Default to sequential.** Step off it only for identity or polarity.
...
- **Never a rainbow.** Magnitude is one hue, light to dark. A multi-hue sequential is legal only for analogous neighbours or a semantic heat scale, and then ships a scale legend.
- **The middle of a diverging scale is never a hue.** Two opposing hues with a neutral gray between them.
- **The [series ladder](#series-ladder) caps the count.** A ninth hue is never minted.
...
| 4. CVD separation | Machado-2009 ΔE ≥ 12 target, ≥ 8 floor, under protanopia and deuteranopia. The floor is legal only with a secondary encoding. Adjacent pairs where only neighbors touch: stacks, bars, lines. Every pair where any two marks can touch: scatter, bubble, choropleth, small multiples; use `--pairs all`. | Validator |
| 5. Contrast against the surface | Marks ≥ 3:1; eased only when a visible label or the table view carries the value | Validator |
| 6. Documented palette only | Every slot traces to a hex in palette.md; nothing eyeballed | Structure |
...
| WARN, CVD 8–12 | Legal only with a secondary encoding: direct labels, gaps or texture | 0 |
...
Run once per mode: `--mode dark --surface "#171717"`. Add `--pairs all` for scatter, bubble, map and small multiples. Run an ordinal ramp with `--ordinal`, which swaps in the ramp checks: monotone L, adjacent ΔL ≥ 0.06, light-end contrast ≥ 2.0:1, one hue throughout.
...
| A sequential or diverging ramp | Lightness climbs monotonically. The categorical validator FAILs a ramp on purpose; leave a good ramp alone. |

## Snap to passing

To land on any design system's own ramps:
...
The categorical channel degrades as series pile up.
...
| 7–8 | Hard ceiling; beyond it sweep the tail into "Other", use small multiples, or double-encode hue × shape |

## Status

Status stands apart from the theme: good → warning → serious → critical. Its steps sit clear of the categorical slots. It always shows with an icon and a label. A series whose meaning is good or bad (error rate, pass/fail) draws from status tokens. A series that is merely fourth in line draws from categorical. One chart never mixes the two. The mapping to `--color-success`, `-warning`, `-error` and `-info` is in [palette.md](palette.md#status-palette).

## Ink

Text takes the text tokens: primary, secondary or muted ink. Never ink a value, a label, a legend entry or an axis in a series hue. The colored mark beside the text identifies the series. One exception: a label inside a fill flips between white and ink by the fill's luminance.

## Texture

Texture is the fallback identity channel for deep CVD, grayscale print and `forced-colors`. Use one directional hand-drawn fill at 45° plus its 135° mirror, and no other angle. Horizontal and vertical read as gridlines or bars. Ink it tone-on-tone from a darker step of the fill's own ramp, equally loud on every slot. Over a value scale the texture is ordered: rotation advances with magnitude, and the lean encodes the diverging sign. It switches on from the accessibility toggle, in print, or under `forced-colors`. The resting state is off; texture is never decoration.

## What a design system plugs in

The method is system-agnostic. Only these parameters vary; the machinery does not. palette.md is the filled-in file.
...
To onboard a system: fill the rows, run its ramps through the validator, and snap every slot to the nearest passing step. Nothing structural moves.
