...
|---|---|---|---|
| **[0.3.9](./releases/0.3.9.md)** | 2026-09-04 | Plugin `agent-ks` 0.10.0: skills re-cut by domain — `agent-ks-config` (setup, site config, themes, layouts) replaces init and add-section, `agent-ks-blog` split out, `agent-ks-logs` renamed `agent-ks-issue-logs`. No engine change beyond the version. | No. Set `engine_version: "0.3.9"`. Old command names `/agent-ks-init`, `/agent-ks-add-section` are gone. |
