...
| `agent-ks issue new-agent-log <id>` | Scaffold an agent log — `settings.json`, `01_summary.md`, and an empty `02_working/00_index.md` you write as rounds land. `03_debrief/` appears when there is something to hand over; `--parent` nests a child log, whose prefix is `≥ 100` | `--kind <code>` · `--name <slug>` · `--parent <path>` (child log) · `--group <a[/b]>` · `--prefix <NNN>` · `--goal <text>` · `--json` · `--tracker` |
| `agent-ks issue new-iteration <id>` | Open the next iteration file in an agent log's `02_working/`, head pre-filled | `--log <path>` · `--name <slug>` · `--producer` · `--iteration <NN>` · `--goal` · `--inputs <a,b>` · `--unit <kind>` · `--agent <name>` · `--json` · `--tracker` |
...
| `agent-ks check links` | Links between content pages resolve **as the site renders them** (`--section`, `--all`, `--dist`). Needs a build first — `./start build` — because it reads the built output, not the source. `check link-form` is the source-only sibling and is the one to reach for by default |
