...
- [08_style-brief.md](./08_style-brief.md) — the brief for the plain-language pass: three groups, facts frozen, the hunt list, the gates, the report shape
...
- Round four, the plain-language pass, three forks of the orchestrator, one report each: [40_style-tracker.md](./40_style-tracker.md) (issues, qna, issue-logs, quick-idea-note) · [41_style-config-cli-blog.md](./41_style-config-cli-blog.md) · [42_style-artifacts-docs-index.md](./42_style-artifacts-docs-index.md)
...
Round four, after Sid read the skills and found the prose jargon-like. Three forks rewrote wording only, under [08_style-brief.md](./08_style-brief.md). Across the ten skills: 1935 sentences became 2314, the average fell from 10.8 to 9.7 words, sentences over 20 words fell from 132 to 67, over 30 from 32 to 16. The orchestrator read the whole diff and found no fact changed. The forks reported eight doubted facts and left them. Three were real, and the orchestrator fixed them: `10_examples.md` said four pickup steps (five); `01_anatomy.md` said "Guide modal" (the code says panel); `agent-ks-qna/SKILL.md` said memory is dropped when a run ends (it stays with the issue). The other five are wording the reader can live with, or facts the pass could not settle. Gates green after the round: `check skill-links`, `check link-form` on the skills and on this log, `_selftest.mjs`, `check issues`, strict YAML on all ten descriptions.

## Handover

Open, in the deferred tables of the two verdict files. The largest: the user-guide re-sync (`19_issues/`, `18_blogs/`, `16_layout-system/`, `10_configuration/02_env.md`, `data/README.md`); the CLI exit-code unification and self-test checks; the docs sort tuple ignoring a file's own prefix (engine, reorders sidebars, Sid's call); the 35 legacy `01_summary.md` logs; the dump issue's bucket shape. Three single-file skills grew past the old sizes and stay exempt from the cap: blog 1084 body words, quick-idea-note 1244, issue-logs 834 with a reference. The plain-language pass pushed six other SKILL.md bodies past 600 words by the words the splits added, none deleted: artifacts 614, cli 607, docs 647, index-check 619, issues 623, qna 649. Sid moved the cap to 1000 (decision I), so all ten pass. Two references sit a few lines over 150: `kinds.md` 154, `06_subtasks.md` 151. Left as they are. Tagging `v0.3.10` is Sid's. The round-four diff is uncommitted.
