...
- [x] **Build the cache loader** — `agent-ks-engine/src/loaders/issue-dates.ts`. Single `git log --no-merges --name-only --pretty=format:'§%aI' -- <tracker-path>` walk on cold start. Incremental refresh via `git merge-base --is-ancestor`. `.git/HEAD` watcher for invalidation. In-memory map only — no `.cache/` file.
