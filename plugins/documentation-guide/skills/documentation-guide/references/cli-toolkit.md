...
| `docs-guide check legacy-tags [root]` | Retired custom-tags syntax (`:::callout` / `<callout>` / `<tabs>` / `<collapsible>`) anywhere under a content root, each hit with its native replacement (GFM alert, `<details>`, flattened sections). Fenced code examples are skipped. Part of the migration toolkit — see `references/doc-migration.md` |
