...
- [ ] **Re-read the shipped code** (the exported status/category constant, the
...
- [ ] **User-guide `19_issues/`** — every page mentions only the shipped vocabulary;
      the hard-error message quoted in docs matches the real one verbatim; UI
      screenshots/descriptions match the category tabs.
- [ ] **`guide.ts`** — legend matches shipped statuses, colors, and rules.
- [ ] **Skill plugin (both skills, repo + cache)** — SKILL.md router rules, all
...
- [ ] **Repo `CLAUDE.md`** — tracker-mental-model paragraph consistent.
- [ ] **The tracker itself** — this issue's own files (issue.md, notes, brainstorm
      resolution markers) reflect what shipped; all trackers pass
      `docs-guide check issues`; full build green.
- [ ] **Fix what the sweep finds** directly (small drifts) or file subtasks (real
