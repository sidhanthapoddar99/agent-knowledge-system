...
The plugin ships one entrypoint, **`docs-guide`**, on `PATH`. Tracker work uses the
`issue` group — `docs-guide issue list / show / subtasks / agent-logs / set-state /
add-comment / add-agent-log / review-queue` — plus `docs-guide check issues`,
`docs-guide find`, and `docs-guide move` (link-aware). Discover with `docs-guide help`;
