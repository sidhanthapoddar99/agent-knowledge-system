#!/usr/bin/env bun
/**
 * agent-ks img — optimize images for documentation so the repo stays small.
 *
...
function usage(code) {
  ...
  out('Usage: agent-ks img <inputs...> [flags]\n');
  ...
  out('  Tip: Playwright (if installed) can capture web screenshots; agent-ks img then shrinks them.');
  process.exit(code);
}
...
if (o.dryRun) {
  console.log('# agent-ks img (dry-run)\n');
...
if (!o.quiet) {
  ...
  console.log(`\n# agent-ks img — ${rows.length} image(s)${o.out ? ` → ${o.out}` : ' (in-place)'}\n`);
  for (const r of rows) {
