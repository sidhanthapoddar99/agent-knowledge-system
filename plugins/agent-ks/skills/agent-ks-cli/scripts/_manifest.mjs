...
// Shared by `issue new-round` and its alias `issue new-iteration`.
const NEW_ROUND_FLAGS = [
  ...
  { name: 'report', desc: 'this file is one report produced inside the current round — takes the next digit 1–9' },
  { name: 'round', value: 'N', desc: 'force the round number instead of deriving it' },
  { name: 'goal', value: 'text', desc: 'the lead paragraph: why this round exists' },
  { name: 'inputs', value: 'a,b', desc: 'comma-separated paths, each written to `# 03 References` as a link with the file title as text' },
...
export const MANIFEST = [
  ...
  // ---- issues (group: issue) ------------------------------------------------
  {
    ...
    flags: [
      { name: 'status', value: 'vals', desc: 'open,blocked,in-progress,input-needed,review,done,dropped,superseded (default: not-Closed)' },
      ...
      { name: 'include-closed', desc: 'widen the default scope to include done/dropped/superseded' },
  ...
  },
  {
    ...
    summary: 'Scaffold an agent log — settings.json and 01_summary.md from templates/log-summary.md',
    flags: [
      ...
      { name: 'goal', value: 'text', desc: 'the lead paragraph of 01_summary.md: why this run exists' },
      { name: 'json', desc: 'structured JSON output' },
      { name: 'tracker', value: 'path', desc: 'non-default tracker' },
    ],
  },
  {
    bin: 'docs-new-round', group: 'issue', verb: 'new-round', category: 3, script: 'issues/new-round.mjs', runtime: 'mjs',
    summary: 'Add a round file flat in an agent log from templates/log-round.md (10, 20, 30; --report writes 21–29)',
    flags: NEW_ROUND_FLAGS,
  },
  {
    bin: 'docs-new-iteration', group: 'issue', verb: 'new-iteration', category: 3, script: 'issues/new-round.mjs', runtime: 'mjs',
    summary: 'Alias of `issue new-round`',
    flags: NEW_ROUND_FLAGS,
  },
  {
    ...
    flags: [
      ...
      { name: 'overview', value: 'text', desc: 'the lead paragraph of overview.md: why this plan exists' },
  ...
  },
  {
    ...
    flags: [
      ...
      { name: 'status', value: 'state', desc: 'one of the canonical eight (default open)' },
      ...
      { name: 'subtask', value: 'a,b', desc: 'comma-separated subtasks (number, slug, or path); each becomes one plain link with the subtask title as text' },
      { name: 'json', desc: 'structured JSON output' },
      { name: 'tracker', value: 'path', desc: 'non-default tracker' },
    ],
  },
  {
    ...
    summary: 'Scaffold a subtask from templates/subtask.md (title + status, lead paragraph, five numbered sections)',
    flags: [
      { name: 'name', value: 'slug', desc: 'kebab-case subtask name, sanitised to [a-z0-9-] — required unless --index' },
      { name: 'title', value: 'text', desc: 'frontmatter title (default: de-kebabed name)' },
      { name: 'group', value: 'a[/b]', desc: 'nest under a grouping folder path (created if missing)' },
      { name: 'overview', value: 'text', desc: 'the lead paragraph: why this subtask exists' },
      { name: 'index', desc: 'write the group index leaf 00_<name>.md (default name: overview) from the same template' },
      { name: 'json', desc: 'structured JSON output' },
      { name: 'tracker', value: 'path', desc: 'non-default tracker' },
    ],
  },
  ...
  },
  {
    ...
    flags: [
      ...
      { name: 'template', desc: 'check the five template headings on subtasks, stages, plan overviews, log summaries and rounds (also on via root settings `template: true`)' },
      { name: 'tracker', value: 'path', desc: 'non-default tracker' },
    ],
  },
  {
