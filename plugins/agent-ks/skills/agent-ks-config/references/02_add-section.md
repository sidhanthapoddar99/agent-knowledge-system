...
> No agent-knowledge-system project detected here (no `config/site.yaml` found walking up). Set one up first: [01_new-project.md](./01_new-project.md).
...
Use the argument when there is one: `/agent-ks-config section <name>`. Otherwise ask:
...
Convert the name to Title Case: `user-guide` gives `User Guide`. Confirm: "Sidebar label: `<Title>`. Accept or override?"
...
- `05_<slug>.md`: the next page; leave gaps, so later inserts need no renumbering
...
Pages inside the section are the docs skill's work from here on.
