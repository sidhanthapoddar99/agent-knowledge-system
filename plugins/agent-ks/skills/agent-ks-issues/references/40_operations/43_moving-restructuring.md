...
Issues, subtasks, notes, and comments cross-link each other with **relative** paths (the rule: [writing.md → Linking](../../../agent-ks-docs/references/writing.md#linking)). A plain `mv` / `git mv` moves the file but leaves every relative link pointing at the old location — inbound links from sibling files break, and relative links *inside* the moved file now resolve from the wrong directory. Nothing warns you; the build still passes.
