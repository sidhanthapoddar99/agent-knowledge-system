...
Every content folder in this project, one row each. This file is the map: read it
first when you need to know where something lives, and add a row whenever you add
a top-level folder. A folder becomes a route through a `pages:` entry in
`../config/site.yaml`.
...
and a starter page, and it registers the route in `../config/site.yaml`. Then add
the row here.
