...
- [x] `04_setup/06_lifecycle-and-review.md` — the new model in full: category/status
      ...
      paragraph.
- [x] `04_setup/02_vocabulary.md` — statuses are now **fixed in code** (only colors
...
- [x] `02_design-philosophy.md` — record the policy reversal honestly: why the
...
- [x] **Remove the assignee-derived in-progress signal everywhere:**
...
- [x] `05_sub-docs/03_subtasks.md` — subtasks share the issue enum AND the field
      name `status` (post subtask-07 rename); new state table; inline-question
      convention for `input-needed`.
- [x] `09_using-with-ai.md` — agent rules under the new model (auto in-progress,
      input-needed protocol, ceiling = Review category, default search scope =
      everything not Closed).
- [x] Sweep `19_issues/` for any remaining `closed`/`cancelled`-as-status,
