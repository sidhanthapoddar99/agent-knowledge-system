...
- **⚠️ RUN TESTS UNDER BUN, not node.** The plugin ships **no `node_modules`**; `gray-matter` exists only in `agent-ks-engine/node_modules`. Under `node`, every `issues/*` command crashes on `import 'gray-matter'` (ERR_MODULE_NOT_FOUND); under `bun` it resolves from bun's global cache. The shims already prefer bun→node; the self-test harness now auto-detects bun→node to match. `bun --version` = 1.3.14 via mise.
- **True baseline self-test: 30/54** under bun (the earlier "22/54" was a node artifact — issues commands were crashing on import, not running). Target: 54/54 (+ checks added as commands land). Remaining failures are genuine contract gaps: `--help`→stderr (want stdout), exit 1 on required-arg commands (want 0 for `--help`), `-h` unhonored. `docs-move`/`docs-img` already pass 4/4. (Harness grows as commands land: 67/67 after 06, 93/93 after 10.)
- **⚠️ `docs` BINARY COLLISION (surfaced subtask 10):** this machine already has a `docs` on PATH (`/mnt/c/.../CUDA/v11.8/docs`). The model-B bare `docs` dispatcher shim can collide — whichever is earlier on PATH wins. The flat `docs-*` aliases are collision-safe (the whole reason for the prefix). CAVEAT to surface at close-out: keep `docs` as a convenience (document the collision + PATH-order caveat) vs rely on flat aliases. Flat aliases stay the safe primary; `docs <group> <verb>` is the ergonomic bonus. Note in subtask 16 docs.
- **Docs-content group is `doc` (singular)**, parallel to `issue`/`blog` — avoids the `docs docs list` stutter. So: `docs doc list/show/search`, `docs blog list/show/search`, bins `docs-doc-*` / `docs-blog-*`. Shared core: `_content.mjs`.
