...
The procedure behind [the index-check skill](../SKILL.md). Terms: an index is a file that lists other files and makes claims about them. Direction A follows the index to the files. Direction B lists the files and compares the listing with the index. Closed means `done`, `dropped` or `superseded`.
...
List two levels deep before you open the index. If you read the index first, it tells you what to expect. Then you find only that.
...
"I could not list the directory" is not a result. It means there is a tool you have not tried. Say which tool you listed with. Write the listing out, so it is a record and not a memory.
...
|---|---|---|
| B | Strike every entry the index names off the listing, one by one. The entry set is what section 2's table names for the kind: for a log, the files and folders beside the leaf. A folder is either named or `MISSING`. The files inside it are not entries. So a sentence about a folder clears the folder only. What is left over is the finding | `MISSING` |
| A | The entry is in both, and the two disagree. This includes every prose claim about state: "NOT DONE", "still open", "parked", "not swept". An unticked `[ ]` box is a claim of the same kind. It is `STALE` only against a target at `review`, `input-needed` or closed. See check 2 | `STALE` |
...
Direction B is the same move for each kind: list, strike off, report the rest. Two kinds need a second listing. For a log's index leaf, `Grep` the log folder for `subtasks/` and `plans/`. Collect the paths those matches link to. Report every one the `Serves:` line does not name. For `notes/` and `brainstorm/` cross-references, list both folders. Report every file that no index, note or `issue.md` points at. Direction A differs by kind.
...
| `subtasks/<NNN_group>/00_*.md` | an entry the leaf calls open whose file says `review`, `input-needed` or closed. `agent-ks check issues` already reports a leaf whose own `status` is behind its members, across the whole tracker, so leave that claim to the CLI |
| `issue.md`, where it points at its own sections | a mention of a note, a plan, a brainstorm or a log that is not on disk |
| `notes/` and `brainstorm/` cross-references | a pointer to a note that moved, or to a brainstorm that graduated into `notes/`; a "still being decided" whose target records the decision |
...
Check 1 alone carries an exception. A stage's status describes the schedule, not the work. A stage does not have to wait for its subtasks. An all-closed stage that is still open is an observation for a human, never an error. Do not extend this caveat to checks 2 to 5.

## 7. The report

Write the findings first. Then count them and fill the header from that count. Lead with the count and the verdict. Then the findings by direction, B first, even when B is empty. Then the coverage table. Then what you read. `<M>` counts the index files you read a claim out of. For a plan, that is `overview.md` plus every stage that carries a checkbox or a `subtasks:` list. For a log, it is the leaf. For a subtask group, it is the `00_*.md` leaf. For an issue folder, it is every index you checked across the kinds. The `ORPHAN` findings from `agent-ks check link-form` count toward `<N>`. Quote each one with the file and the line the script gave, so the reader can check it.
...
A whole issue folder holds several kinds, so it gets one report and not one per kind. Write a single Direction B section for the folder. Then write one Direction A subsection per kind, headed by the kind. Keep one coverage table and give it a leading `kind` column. Cap each kind at five findings, and say how many you folded. Then a large folder still returns a report a reader can read in one sitting.

Finish with what you read: index files checked and targets resolved, as counts. Name what you did not reach. A clean result is a real answer. Say "no disagreements found" and list what you checked. Do not invent a finding.
